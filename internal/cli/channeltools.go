	...
	tools := []channel.Tool{
		...
		},
		{
			...
			Handler: func(ctx context.Context, args json.RawMessage, _ func(string)) (string, bool) {
				...
				if err := rc.SubmitOrganization(ctx, session, scope, org, in.VersionNumber, in.SectionKey, in.Partial); err != nil {
					return err.Error(), true
				}
				return "ok", false
			},
		...
		},
		{
			...
			Description: "List the open cc-review's current version_number, whether it is a stack, and its ordered sections (trunk-most first, the pending working-tree section last). Each section carries its section_key (\"\" for pending, else the branch), branch/parent_branch, pending flag, patch_path (the on-disk unified diff), review_files_path (the file list as JSONL — one {path,status,reviewed,hidden} per line), an organized flag, and, when a prior same-key organization exists, organization_path (that org delta-annotated: overview + chapters with basis_version, per-file delta changed/moved/removed and new_paths — rebuild context). Organize every section whose organized flag is false, submitting per section_key. Read the paths from disk. A small file set — or a status/reviewed/hidden-filtered subset — is also inlined per section as files with match_count.",
		...
		"properties": map[string]any{
			"files": map[string]any{
				...
				"items": map[string]any{
					"type": "object",
					"properties": map[string]any{
						"section_key": map[string]any{"type": "string", "description": "the file's section (a stack branch, or \"\" for the pending working-tree section); from get_review_files"},
					...
					"required": []string{"section_key", "path"},
				},
			},
...
func submitOrganizationToolSchema() map[string]any {
	return map[string]any{
		...
		"properties": map[string]any{
			...
			"section_key":    map[string]any{"type": "string", "description": "the section this organization covers (a stack branch, or \"\" for the pending working-tree section); from get_review_files"},
		...
		"required": []string{"chapters", "version_number", "section_key"},
	}
}
		...
		"properties": map[string]any{
			"items": map[string]any{
				...
				"items": map[string]any{
					...
					"required": []string{"kind", "section_key", "file_path", "side", "start_line", "end_line"},
				},
			},
