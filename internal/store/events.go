...
// channel.changed event reports connected:true.
func (s *Store) StaleConnectedReviews(ctx context.Context) ([]string, error) {
	rows, err := s.db.QueryContext(ctx, `
		SELECT review_id FROM events
		WHERE type=? AND json_extract(payload,'$.connected')=1
		  AND seq=(SELECT MAX(seq) FROM events e2 WHERE e2.review_id=events.review_id AND e2.type=?)`,
...
func (s *Store) MaxEventSeq(ctx context.Context, reviewID string) (int64, error) {
	...
	if err := s.db.QueryRowContext(ctx,
		`SELECT COALESCE(MAX(seq),0) FROM events WHERE review_id=?`, reviewID).Scan(&seq); err != nil {
