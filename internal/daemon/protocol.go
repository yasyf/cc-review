...
// drain on shutdown) but with a flock guarding lazy start and no version-skew
// eviction. Every control op is fast request/response; realtime delivery is the
// HTTP SSE stream, not a blocking socket op.
package daemon
