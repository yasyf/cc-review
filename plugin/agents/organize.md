...
2. `get_review_files` — pass a `status`/`reviewed`/`hidden` filter to pull just the files the request targets (`status: "M"` for modified, `reviewed: false` for the unreviewed set); read `review_files_path` for the full list when no filter fits. Then apply in one batch:
...
- A re-organization request ("split the API chapter", "group by risk") → rebuild from `organization_path` and submit_organization again, reflecting the instruction in the changed summaries, then close the request.
