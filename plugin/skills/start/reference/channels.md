...
1. Approve cc-review's channel once — run `cc-review setup-channels --apply`, or accept the offer `/cc-review:start` makes on first run (see `channels-setup.md`). This moves the plugin onto Claude's approved allowlist, so it loads with no dev-channels warning.
...
`start` detects the channel and prints `channel: active` as the second of its output lines (URL, `channel:`, `setup:`, and `organize:` on a new version), which tells the skill to skip the Monitor — events arrive once, as channel tags, not twice.
