#!/bin/sh
# GENERATED by cc-guides from .claude/fragments/plugin/scripts/render-descriptor.sh/ — do not edit; edit the fragments and push — CI renders.
# Render descriptor/cc-review.binrun.tmpl into a concrete cc-review.binrun,
...
# "cc-review_<version>_<os>_<arch>.tar.gz", the shape binrun ships. A plugin
...
here="$(cd "$d/.." && pwd)"
tmpl="$here/descriptor/cc-review.binrun.tmpl"
...
for pair in MACOS_AARCH64:darwin_arm64 MACOS_X86_64:darwin_amd64 LINUX_X86_64:linux_amd64 LINUX_AARCH64:linux_arm64; do
  key="${pair%%:*}"
  osarch="${pair##*:}"
  name="cc-review_${version}_${osarch}.tar.gz"
...
[ "$matched" -gt 0 ] || { echo "render-descriptor: no cc-review_${version}_<os>_<arch>.tar.gz archives in $checksums" >&2; exit 1; }
